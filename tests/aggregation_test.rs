...
use declarative_dataflow::plan::{Aggregate, AggregateMulti, AggregationFn, Join, Project};
...
fn multi() {
    timely::execute(Configuration::Thread, move |worker| {
        ...
        // [:find (max ?amount) (min ?debt) (sum ?amount) (avg ?debt) (variance ?debt)
        //  :where [?e :amount ?amount][?e :debt ?debt]]
        let (e, amount, debt) = (1, 2, 3);
        let plan = Plan::AggregateMulti(AggregateMulti {
        ...
        let plan_group = Plan::AggregateMulti(AggregateMulti {
