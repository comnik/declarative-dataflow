...
use parquet::column::writer::{ColumnWriter, ColumnWriterImpl, get_typed_column_writer};
...
fn main() {
    ...
    for readline in in_file.lines() {
        ...
        if !line.starts_with('#') && line.len() > 0 {
            ...
            match typ {
                "n" => { node_writer.write_batch(&[src], None, None).unwrap(); },
    ...
    row_group_writer.close_column(ColumnWriter::Int64ColumnWriter(node_writer)).unwrap();
